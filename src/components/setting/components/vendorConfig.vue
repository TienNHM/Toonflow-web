        ...
        <t-form :data="currentVendor" labelAlign="top">
          ...
          <t-form-item v-for="input in requiredInputs" :key="input.key" :name="input.key">
            ...
            <t-input
              ...
              :type="input.type"
              :placeholder="getInputPlaceholder(input) || $t('settings.vendor.inputPlaceholder')"
              clearable
              @blur="onBlurFn">
    ...
    <t-dialog
      ...
      <div class="data">
        <t-radio-group variant="default-filled" v-model="addMode">
          <t-radio-button value="importAdd">{{ $t("settings.vendor.importByFile") }}</t-radio-button>
          <t-radio-button value="linkAdd">{{ $t("settings.vendor.importByLink") }}</t-radio-button>
          <t-radio-button value="codeAdd">{{ $t("settings.vendor.importByCode") }}</t-radio-button>
        </t-radio-group>
        <div class="linkAdd" v-if="addMode == 'linkAdd'">
          <t-alert theme="warning" style="margin-bottom: 20px">
            {{ $t("settings.vendor.linkImportHint") }}
          </t-alert>
...
async function getVendorList() {
  loading.value = true;
  try {
    const res = await axios.post("/setting/vendorConfig/getVendorList", { locale: locale.value });
            ...
            if (!data.includes("vendor")) {
              ...
              if (data.includes("<html>")) {
                alertBox = DialogPlugin.alert({
                  theme: "danger",
                  header: $t("settings.vendor.msg.linkReturnedWebpage"),
                  body: $t("settings.vendor.msg.linkReturnedWebpageBody"),
                  onConfirm: ({ e }) => {
                    alertBox.hide();
                  },
                });
                ...
                DialogPlugin.alert({
                  theme: "danger",
                  header: $t("settings.vendor.msg.linkInvalidContent"),
                  onConfirm: ({ e }) => {
                    alertBox.hide();
                  },
                });
