<template>
  <t-dialog
    ...
    :closable="false"
    width="40vw"
    :header="props.formData.id ? $t('workbench.assets.add.editHeader') : $t('workbench.assets.add.addHeader')"
    :maskClosable="false"
        ...
        <t-form-item :label="$t('workbench.assets.add.audioFile')" name="audioFile">
          <div class="audio-list">
            <div v-for="(item, index) in audioItems" :key="index" class="audio-item">
              ...
              <t-input v-model="item.describe" :placeholder="$t('workbench.assets.add.audioDescribePh')" class="audio-text-input" />
            </div>
            <t-button theme="primary" variant="outline" size="small" @click="addAudioItem">
              <template #icon><i-plus /></template>
              {{ $t("workbench.assets.add.addAudioItem") }}
            </t-button>
          </div>
        </t-form-item>
      </t-form>
